...
- **Variables:** `APPLE_TEAM_ID`, `BUNDLE_IDENTIFIER`, **`XC_TARGET_NAME`** (must be `BaseProject`), `MATCH_GIT_URL`, `LAST_UPLOADED_BUILD_NUMBER`, `APPLE_APP_ID`

After renaming the app to BaseProject, ensure **Variables → XC_TARGET_NAME** is set to `BaseProject`.
